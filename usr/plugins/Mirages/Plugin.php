...
class Mirages_Plugin implements Typecho_Plugin_Interface {
    ...
    const VERSION = 71002.99;
    const VERSION_TAG = "7.10.2";
    ...
    public static function activate() {
        ...
        Typecho_Plugin::factory('admin/write-js.php')->write = array('Mirages_Plugin', 'writeJs');


        $conflictedPlugins = Mirages_ConflictHelper::loadConflictedPlugins();
        if (!empty($conflictedPlugins)) {
            return _t('检测到插件冲突，请到 Mirages 插件设置页面查看详情');
        }
    }
    ...
    public static function config(Typecho_Widget_Helper_Form $form) {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'resolveConflict') {
            $message = Mirages_ConflictHelper::resolveConflict();
...
JAVASCRIPT;
        ...
        $conflictedPlugins = Mirages_ConflictHelper::loadConflictedPlugins();
        $showBanner = $needShowRestartBanner || !empty($conflictedPlugins);
        if ($showBanner) {
            echo <<<HTML
            <style>
            .notice-block {
...
HTML;
        ...
        if (!empty($conflictedPlugins)) {
            $writable = @Mirages_Utils::is_really_writable(Mirages_Utils::pluginsDir());
            if ($writable === TRUE) {
                $resolveUrl = Typecho_Common::url('/options-plugin.php?config=Mirages&action=resolveConflict', Helper::options()->adminUrl);
                $content = <<<HTML
                <script type="text/javascript">
                    function doResolveConflict() {
                        if (confirm("**以下内容必读**\\n\\n1. 插件冲突会修改您插件的部分代码，强烈建议您在使用此功能前先备份相关插件！\\n2. 请在处理完成后「禁用并重启」仍然提示冲突的插件!\\n\\n现在，确定要执行「插件冲突自动处理」吗？")) {
                            window.location.href = "${resolveUrl}";
                        }
                    }
                </script>
                <p class="content">您可以选择<a href="#" onclick="doResolveConflict()">自动处理插件冲突</a>完成冲突处理<br>或参考<a href="https://mirages.docs.get233.com/#/docs/advance/plugin-conflict-solution" target="_blank">插件冲突解决方案</a>手动处理后重试</p>
HTML;

            } else {
                $content = <<<HTML
                <p class="content">[因插件目录没有写入权限，插件冲突自动处理功能已停用]<br>请参考<a href="https://mirages.docs.get233.com/#/docs/advance/plugin-conflict-solution" target="_blank">插件冲突解决方案</a>手动处理后重试</p>
HTML;
            }

            $plugins = join("<br>", $conflictedPlugins);
            echo <<<HTML
            <div class="notice-block">
                <p class="title">检测到插件冲突</p>
                <p class="content">${plugins}</p>
                ${content}
                <p class="content">(如果您使用手动或自动方式处理完成后仍然提示冲突，请重启以上插件)</p>
            </div>
HTML;
        }
    ...
    private static function checkIfNeedRestartPlugin() {
        ...
        if (!empty($handles)) {
            ...
            if (!array_key_exists('admin/write-js.php:write', $handles)) {
                return true;
            }
        }
    ...
    public static function writeBottom() {
        ...
        if (Mirages_Utils::isTrue(self::$themeOptions->enableVditor)) {
            echo <<<VDITOR
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/vditor@3.7.1/dist/index.css" />
<script src="https://cdn.jsdelivr.net/npm/vditor@3.7.1/dist/index.min.js" defer></script>
<script src="${pluginBaseUrl}js/pinyin.min.js?v=${version}"></script>
VDITOR;
        }
        echo <<<HTML
<script type="text/javascript" src="${pluginBaseUrl}js/dashboard.write.min.js?v=${version}"></script>
HTML;

    }
    public static function writeJs() {
        $security = Typecho_Widget::widget('Widget_Security');
        $uploadBaseUrl = $security->getIndex('/action/upload');
        if (Mirages_Utils::isTrue(self::$themeOptions->enableVditor)) {
            echo <<<JAVASCRIPT
<script type="text/javascript">
(function () {
    LocalConst.BASE_UPLOAD_URL = '${uploadBaseUrl}';
    $(document).ready(function () {
        $('form[name=write_post],form[name=write_page]').off('submit');
    });
})();
</script>
JAVASCRIPT;
...
//        $content = self::_doUnEscapeTexBlock($content);
    ...
    private static function _renderCheckbox($content) {
        return preg_replace_callback('/<li>\s*\[(\s|x|X)]\s*([^<>]+)<\/li>/i', function ($match) {
            $checked = $match[1];
            $text = $match[2];
        ...
        self::$shortcodeTags['hide'] = function ($content, $params, $tag) {
            ...
            if (!array_key_exists($key, self::$shortcodeStats)) {
                ...
                if ($user->hasLogin()) {
                    ...
                    if (self::themeAdvancedSettingIsTrue('embedCommentOptions__comment2ViewStrict')) {
