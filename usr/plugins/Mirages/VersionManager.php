...
class Mirages_VersionManager {
    ...
    public static function requestUpdate() {
        ...
        if (array_key_exists('domainToken', $package) && !empty($package['domainToken']) && strlen($package['domainToken']) > 20) {
            $db = Typecho_Db::get();
            ...
            if (!empty($currentThemeConfigRow) && !empty($currentThemeConfigRow['value'])) {
                $themeConfig = unserialize($currentThemeConfigRow['value']);
                $themeConfig['k'] = $package['domainToken'];
                $themeConfig = serialize($themeConfig);
                $backup = array(
                    'value' => $themeConfig
                );
...
//        }
    ...
    private static function loadUpdatePackage() {
        ...
        $requestURL = "https://store.get233.com/api/mirages/requestUpdate";
        mLog("正在获取更新下载地址...");
