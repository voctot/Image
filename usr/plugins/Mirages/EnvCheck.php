...
class Mirages_EnvCheck {
    ...
    private static function doCheckIDN() {
        if (function_exists('idn_to_ascii')) {
            mLog('PHP - intl 扩展: 正常', Mirages_Utils::LOG_LEVEL_SUCCESS);
        } else {
            mLog('PHP - intl 扩展: 没有检测到 intl 扩展，域名授权过程中，国际化域名（例如中文域名）将不受支持', Mirages_Utils::LOG_LEVEL_WARNING);
        }
    }
