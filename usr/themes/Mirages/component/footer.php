...
<?php
    ...
    if (Utils::hasValue(Mirages::$options->beian)) {
        ...
        $copyright = "<p>{$copyright}</p><p>{$beiAn}</p>";
    } else {
        $copyright = "<p>{$copyright}</p><p> {$beiAn}</p>";
    }
?>
<footer id="footer" role="contentinfo">
    <div class="container"  data-index="<?php echo (Mirages::$options->timeValid - intval(Mirages::$options->timeValid / 1170) * 1170)?>">
        <?php echo $copyright;?>
    </div>
</footer>
...
<?php if (Mirages::$options->enableMathJax): ?>
...
<?php if (Mirages::$options->enableFlowChat): ?>
...
<?php if (Mirages::$options->enableMermaid): ?>
...
<?php
    $needLoadWebFont = USE_GOOGLE_FONTS || USE_SERIF_FONTS || strpos(Mirages::$options->bodyClass, 'serif-fonts') !== FALSE;
    $families = '';
    if (USE_SERIF_FONTS || strpos(Mirages::$options->bodyClass, 'serif-fonts') !== FALSE) {
...
<script>Mirages.loadWebFont([<?php echo $families ?>]);</script>
<?php endif?>
